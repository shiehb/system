...
export default function AddEstablishment({
  ...
  const [polygon, setPolygon] = usePersistedState<PolygonData | null>(
    "est_polygon",
    null
  );
  const [restrictPinToPolygon, setRestrictPinToPolygon] = usePersistedState(
    "est_restrictPin",
    false
  );
  ...
  const polygonArea = polygon
    ? calculatePolygonArea(polygon.coordinates[0])
    : 0;
  ...
  const handlePolygonChange = (newPolygon: PolygonData | null) => {
    ...
    if (newPolygon) {
      ...
      toast.success(`Polygon boundary created (${formatArea(area)})`);
  ...
  const confirmSave = async () => {
    ...
    const formData: EstablishmentFormData = {
      ...
      nature_of_business_id: natureOfBusiness
        ? Number.parseInt(natureOfBusiness)
        : undefined,
  ...
  const handleCreateBusinessType = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNatureOfBusiness(createdType.id.toString());
                    ...
                    >
                      ...
                      {polygon && (
                        <Badge variant="secondary" className="ml-1 text-xs">
                          {polygon.coordinates[0].length} pts
                        </Badge>
                      )}
                    </TabsTrigger>
                  </TabsList>
                        ...
                        {polygon && (
                          <div className="flex items-center gap-2">
                            <Badge variant="outline" className="text-xs">
                              {polygon.coordinates[0].length} vertices
                            </Badge>
                    ...
                    {polygon && (
                      <Alert>
                        ...
                        <AlertDescription>
                          ...
                          {polygon.coordinates[0].length} vertices covering
                          approximately {formatArea(polygonArea)}.{" "}
                          {restrictPinToPolygon &&
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              entered data including any polygon boundary.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              including any polygon boundary.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
